<div class="">
    ...
    @if ($pins->count() > 0)
        <article class="mt-4">
            <div class="relative py-3 px-3 sm:px-6 sm:py-6 border-2 border-skin-500 rounded-lg ">
                <div
                    class="absolute top-0 -left-0  text-white capatalize py-2 px-4 leading-6  inline-flex flex-row justify-center items-center no-underline rounded-br-lg font-semibold cursor-pointer transition duration-200 ease-in-out shadow-skin-100 bg-skin-500">
                    Pinned
                </div>
                <div class="my-10">
                    @foreach ($pins as $pin)
                ...
                <div
                    class="absolute bottom-0 right-0  text-white capatalize py-2 px-4 leading-6  inline-flex flex-row justify-center items-center no-underline rounded-tl-lg font-semibold cursor-pointer transition duration-200 ease-in-out shadow-skin-100 bg-skin-500">
                    Pinned
                </div>
            </div>
        </article>
                ...
                @foreach ($blogs as $blog)
                    <x-cards.blog-card :blog="$blog" :pin="true">
                        <x-buttons.simple wire:click.prevent="pin({{ $blog->id }})" :default=false
                            class="hover:bg-skin-50 hover:text-skin-500  focus:ring-skin-100" :wire:key="$blog->id">
                            <span title="Bookmark this Article">
